...
impl<'inp> Parser<'inp> {
	...
	fn parse_statements(&self, pairs: Pairs<'_, Rule>) -> Vec<Statement> {
		...
		for pair in pairs {
			...
			let statement = match pair.as_rule() {
				...
				rule => {
					error!(
						"statement of type '{:?}' is not yet implemented",
						pair.as_span(),
						rule
					)
				}
	...
	fn parse_var_define_stmt(&self, pair: Pair<'_, Rule>) -> ast::VariableDefine {
		...
		let name = pair_into_string(&name_pair);
	...
	fn parse_object(&self, pair: Pair<'_, Rule>) -> expressions::Object {
		...
		for entry_pair in pairs {
			...
			entries.push(self.parse_object_entry(entry_pair));
		}
	...
	fn parse_function_call(&self, pair: Pair<'_, Rule>) -> expressions::Call {
		...
		let next_pair =
			get_pair_safe!(from pairs expect call_body_empty | call_body_nonempty in pair);
		...
		let function = self.parse_expression(get_pair_safe!(from pairs expect expression in pair));
	...
	fn parse_while_stmt(&self, pair: Pair<'_, Rule>) -> ast::WhileLoop {
		...
		let condition = self.parse_expression(get_pair_safe!(from pairs expect expression in pair));

		let statements =
			self.parse_statements(get_pair_safe!(from pairs expect do_block in pair).into_inner());
	...
	fn parse_if_stmt(&self, pair: Pair<'_, Rule>) -> ast::If {
		...
		let statements = self
			.parse_statements(get_pair_safe!(from pairs expect statements in pair).into_inner());
	...
	fn parse_expr_assign_stmt(&self, pair: Pair<'_, Rule>) -> ast::ExpressionAssign {
		...
		let expression =
			self.parse_expression(get_pair_safe!(from pairs expect expression in pair));

		let value = self.parse_expression(get_pair_safe!(from pairs expect expression in pair));

		ast::ExpressionAssign {
			expression,
			value,
			position
		}
	...
	fn parse_term(&self, pair: Pair<'_, Rule>) -> expressions::Term {
		...
		match pair.as_rule() {
			...
			Rule::binary_expression | Rule::expression => {
				return self.parse_expression(pair).into()
			}
		...
		match inner_pair.as_rule() {
			...
			Rule::number_literal | Rule::string_literal | Rule::boolean_literal => {
				self.parse_literal(inner_pair).into()
			}
	...
	fn parse_function_def_args(&self, pair: Pair<'_, Rule>) -> Vec<(String, Position)> {
		...
		for arg_pair in pairs {
			...
			arguments.push((pair_into_string(&arg_pair), position));
		}
	...
	fn parse_unary_expression(&self, pair: Pair<Rule>) -> expressions::Unary {
		...
		let operator =
			expressions::operators::UnaryOperator::try_from(pair_into_string(&operator_pair))
				.unwrap();

		let operand = self.parse_term(get_pair_safe!(from pairs expect term in pair));
	...
	fn parse_string_literal(&self, pair: Pair<Rule>) -> expressions::Literal {
		...
		expressions::Literal::String(interpreted_literal, position)
	}
